...
import { useTranslatedCommunities } from "../hooks/useTranslatedCommunities";
...
export default function CommunityPage() {
  ...
  const { t, translateCommunity } = useTranslation();
  const [mobileView, setMobileView] = useState("list"); // "list" or "chat"
  
  // Data State
  const [groups, setGroups] = useState([]);
  const { groups: translatedGroups, isTranslating } = useTranslatedCommunities(groups);
  ...
  const handleJoin = async (communityId) => {
    try {
      ...
      fetchCommunities();
      const joinedGroup = translatedGroups.find(g => g._id === communityId);
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {translatedGroups.length === 0 ? (
               <div className="p-8 text-center text-emerald-600">
                 ...
                 <p className="font-semibold">{t('noCommunities')}</p>
        ...
        >
          {selectedGroup ? (
                          ...
                          >
                              ...
                              <div className="px-6 pb-6 -mt-10">
                                  ...
                                  <div className="mt-4 p-4 bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl border border-emerald-100">
                                      ...
                                      <p className="text-gray-600 text-sm leading-relaxed">{selectedGroup.description || t('noDescription')}</p>
                                  </div>
                                  ...
                                  <div className="mt-4 flex gap-4 text-center">
                                      <div className="flex-1 p-4 border border-emerald-100 rounded-2xl bg-emerald-50">
                                          ...
                                          <div className="text-xs text-emerald-600 uppercase font-semibold mt-1">{t('membersLabel')}</div>
                                      </div>
                                      <div className="flex-1 p-4 border border-emerald-100 rounded-2xl bg-emerald-50">
            ...
            <div className="hidden md:flex flex-col items-center justify-center h-full text-center p-8 bg-gradient-to-br from-emerald-50 to-green-50">
               ...
               <h2 className="text-4xl font-bold text-gray-800 mb-3">{t('communities')}</h2>
               <p className="text-emerald-600 max-w-md text-lg">
                 {t('selectCommunityDescription')}
               </p>
               <div className="mt-8 flex items-center gap-2 text-emerald-500 text-sm font-semibold">
                 <MapPin size={16} /> {t('messagesEncrypted')}
               </div>
            </div>
          )}
        </div>
